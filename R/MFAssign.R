...
MFAssignCHO <- function(peaks, isopeaks = "none", ionMode, lowMW = 100, highMW = 1000, POEx = 0, NOEx = 0, Ex = 0, Mx = 0, NH4x = 0, Zx = 1, Ox = 30,
  ...
  print_warning(POEx > 1, "WARNING: Positive Odd Electron (POEx) is greater than 1, are you sure that is what you want?")
  print_warning(NOEx > 1, "WARNING: Positive Odd Electron (NOEx) is greater than 1, are you sure that is what you want?")
  print_warning(ionMode != "pos" & ionMode != "neg", "WARNING: ionMode should be 'pos' or 'neg' ")
  print_warning(Ox != 30, "WARNING: Ox is not at its default value, this will cause the core formula algorithm to perform additional or fewer loops, are you sure you want it changed?")
  print_warning(ppm_err > 3, "WARNING: The maximum allowed error (ppm_err) is greater than 3, is this what you want?")
...
MFAssign <- function(peaks, isopeaks = "none", ionMode, lowMW = 100, highMW = 1000, POEx = 0, NOEx = 0, Nx = 0, Sx = 0, Px = 0, S34x = 0,
  ...
  components <- factor(
    c(
      "C", "H", "O", "N", "S", "P", "Cl", "Fl", "E", "S34", "N15", "D", "Cl37", "Br", "Br81",
      "I", "M", "NH4", "POE", "NOE", "Z"
    ),
    levels = c(
      "C", "H", "O", "N", "S", "P", "Cl", "Fl", "E", "S34", "N15", "D", "Cl37", "Br", "Br81",
  ...
  records1X <- dplyr::mutate(records1X,
    max_LA = theor_mass1 / 13, actual_LA = ((C - E) + N + S + Fl + Br + Br81 + I + O + E + S34 + P + Cl + Cl37 + N15),
    rule_13 = round(actual_LA / max_LA, 1),
    Senior1 = H + P + N + Cl + Fl + Cl37 + N15 + Br + Br81 + I,
    STest = S + S34, ClTest = Cl + Cl37, NTest = N + N15, DBEO = DBE - O, BrTest = Br + Br81,
    max_H = C * 2 + 2, H_test = round(H / max_H, 1),
  ...
  if (N3corr == "on") {
    ...
    GoodN3OS <- merge(N3OSAlign, Aligned,
      by.x = c("C13_mass1", "C13_Abund1"),
      by.y = c("C13_mass1", "C13_Abund1"), all = T
    ) # LCMS
    ...
    N3OS_rem2 <- merge(Unambig2, N3OS_rem,
      by.x = c("C13_mass", "C13_Abund"),
      by.y = c("Exp_mass", "RA"), all = T
    ) # LCMS
