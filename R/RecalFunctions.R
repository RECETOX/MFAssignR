...
plot_spectrum <- function(plotpeak, RecalPlot) {
    ggplot() + geom_segment(data = plotpeak, size = 0.7, aes_string(x = "mass", xend = "mass", y = 0, yend = "Abundance"), alpha = 0.3, color = "grey57") + geom_segment(data = RecalPlot,
        size = 1.2, aes_string(x = "Exp_mass", xend = "Exp_mass", y = 0, yend = "Abundance"), color = "blue") + theme_bw() + labs(x = "Ion Mass", y = "Abundance", title = "Assignment Mass Spectrum",
        color = "Series") + theme(axis.title = element_text(size = 15, face = "bold"), strip.text = element_text(size = 15, face = "bold"), axis.text = element_text(size = 15, face = "bold"),
        legend.title = element_text(face = "bold", size = 15), legend.text = element_text(face = "bold", size = 15), panel.grid.minor.x = element_blank(), panel.grid.major.x = element_blank(),
        strip.background = element_blank(), plot.title = element_text(size = 16, face = "bold"))
}
...
processKnown <- function(rest, known, kmd_col, z_col, num_col, type_col, type, multiplier, element_mass, step_limit, remove_indices) {
    ...
    step_result <- dplyr::mutate(step_result, form = paste(C, H, O, N, S, P, E, S34, N15, D, Cl, Fl, Cl37, M, NH4, POE, NOE, sep = "_"))
...
#'
#' @export
Recal <- function(df, peaks, isopeaks = NA, mode, SN = 0, mzRange = 30, series1 = NA, series2 = NA, series3 = NA, series4 = NA, series5 = NA, series6 = NA, series7 = NA, series8 = NA,
    series9 = NA, series10 = NA, step_O = 3, step_H2 = 5, CalPeak = 150) {
    cols <- ncol(peaks)
    min <- round(min(peaks[1]))
    max <- round(max(peaks[1]))

    RecalList <- data.frame(series = c(series1, series2, series3, series4, series5, series6, series7, series8, series9, series10), Tag = c("S1", "S2", "S3", "S4", "S5", "S6", "S7",
        "S8", "S9", "S10"))
    ...
    if (cols == 2) {
        isopeaks <- if (any(is.na(isopeaks))) {
            data.frame(mass = 1, Abundance = 1, Tag = "X")
        } else {
            isopeaks
        }
        ...
        # cehck <- Iso %>% filter(!is.na(exp_mass)) Monoisotopic Peaks Prepares the recalibrant masses for use in recalibration steps.
        RecalList <- RecalList2[c("Abundance", "Exp_mass", "C", "H", "O", "N", "S", "P", "E", "S34", "N15", "D", "Cl", "Fl", "Cl37", "M", "NH4", "POE", "NOE", "Z", "C13_mass")]
        RecalList <- processMassList(RecalList)

        Rest <- df[c("Abundance", "Exp_mass")]
        Rest <- processMassList(Rest)

        ############ Picking recalibrants with series Process known_O
        ...
        # Good to here Picking recalibrants by abundance
    ...
    if (cols == 3) {
        isopeaks <- if (any(is.na(isopeaks))) {
            data.frame(mass = 1, Abundance = 1, RT = 0, Tag = "X")
        } else {
            isopeaks
        }
        isopeaks <- data.frame(isopeaks)
        ...
        ########################################## Monoisotopic Peaks Prepares the recalibrant masses for use in recalibration steps.
        RecalList <- RecalList2[c("Abundance", "Exp_mass", "RT", "C", "H", "O", "N", "S", "P", "E", "S34", "N15", "D", "Cl", "Fl", "Cl37", "M", "NH4", "POE", "NOE", "Z", "C13_mass")]
        RecalList <- processMassList(RecalList)
    ...
    ######################################################## Setting up the recalibrant and mass lists so they can be recalibrated by section.  mzRange <- 50
    ...
    for (i in 1:Dummy2) {
        # Sets up the bins for the recalibrants
        I <- i

        NewRecal$Range <- replace(NewRecal$Range, NewRecal$Exp_mass >= min(NewRecal$Exp_mass) + (I - 1) * mzRange & NewRecal$Exp_mass < min(NewRecal$Exp_mass) + I * mzRange, I)
    }

    peaks$Range <- 1

    for (i in 1:Dummy2) {
        # Sets up the bins for the raw data
        I <- i

        peaks$Range <- replace(peaks$Range, peaks$mass >= min(peaks$mass) + (I - 1) * mzRange & peaks$mass < min(peaks$mass) + I * mzRange, I)
    }

    # This section calculates how many recalibrants are in each segment test <- NewRecal
    NewRecal$Test <- 1
    for (i in 1:Dummy2) {
        I <- i
    ...
    ####################################### 33 Fixing the problem with too many recalibrants in a single section. 5/13/20 THIS IS WHERE I AM NOW
    ...
    ####################################### Error message if the problem is having too many recalibrants START FROM HERE

    if (nrow(GapCheck) > 0)
        stop("Gap in recalibrant coverage, try adding more recalibrant series")  # New
    ...
    if (nrow(check) != nrow(check2))
        stop("Increase the value of mzRange")
    ...
    for (j in 1:(Dummy3)) {
        ...
        while (J <= Dummy3) {
            ...
            while (counter == 1) {
                ...
                NewRecal_2$weight <- factorial(nrow(NewRecal_2) - 1)/(factorial(NewRecal_2$num) * factorial((nrow(NewRecal_2) - 1) - NewRecal_2$num))
    ...
    for (i in 1:Dummy2) {
        ...
        FinalRecal3$Range <- replace(FinalRecal3$Range, FinalRecal3$Exp_mass >= min(FinalRecal3$Exp_mass) + (I - 1) * mzRange & FinalRecal3$Exp_mass < min(FinalRecal3$Exp_mass) +
            I * mzRange, I)
    }
    ...
    if (is.na(mean(FinalRecal3$Ejsum)))
        stop("Too many recalibrants in section,
...
RecalList <- function(df) {
    ...
    df <- df[, -which(ncol(df) == 49 & seq_along(df) %in% cols_to_remove_if_49 | ncol(df) == 53 & seq_along(df) %in% cols_to_remove_if_53), drop = FALSE]
    ...
    Recal$M.window <- cut(Recal$Maxmass, breaks, labels = c("0-200", "200-300", "300-400", "400-500", "500-600", "600-700", "700-800", ">800"), include.lowest = TRUE)
...
AddCalculatedSummary <- function(Recal) {
    ...
    Recal <- dplyr::mutate(Recal, Min = min(Exp_mass), Max = max(Exp_mass), MInt = mean(Abundance), Maxmass = ifelse(Abundance == max(Abundance), Exp_mass, NA), Maxint = max(Abundance),
